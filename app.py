...
DESTINATIONS = [
    ...
    {"name": "Lender Request",     "url": "/forms/lender",          "keywords": ["lender","mortgage","bank","financing","loan","lend"]},
    {"name": "Legal Request",      "url": "/forms/legal",           "keywords": ["legal","lawyer","litigation","bylaw","rule","enforcement","court"]},
    {"name": "Strata Documents",   "url": "/forms/strata-docs",     "keywords": ["documents","docs","minutes","bylaws","financials","budget","records","strata documents"]},
    {"name": "Pre-Authorized Debit","url": "/forms/debit",          "keywords": ["debit","pad","payment","fees","strata fees","pre-authorized","automatic payment"]},
...
def ask():
    ...
    if dest:
        return jsonify({"message":f"I can help with that.", "url":dest["url"], "destination_name":dest["name"]})
...
def form_maintenance():
    return form_route("form_maintenance.html","Maintenance Request")

@app.route("/forms/complaint", methods=["GET","POST"])
def form_complaint():
    return form_route("form_complaint.html","Complaint")

@app.route("/forms/registration", methods=["GET","POST"])
def form_registration():
    return form_route("form_registration.html","Owner Registration")

@app.route("/forms/realtor", methods=["GET","POST"])
def form_realtor():
    return form_route("form_realtor.html","Realtor Request")

@app.route("/forms/lender", methods=["GET","POST"])
def form_lender():
    return form_route("form_lender.html","Lender Request")

@app.route("/forms/legal", methods=["GET","POST"])
def form_legal():
    return form_route("form_legal.html","Legal Request")

@app.route("/forms/strata-docs", methods=["GET","POST"])
def form_strata_docs():
    return form_route("form_strata_docs.html","Strata Documents Request")

@app.route("/forms/debit", methods=["GET","POST"])
def form_debit():
    return form_route("form_debit.html","Pre-Authorized Debit")

@app.route("/forms/form-k", methods=["GET","POST"])
def form_k():
    return form_route("form_k.html","Form K")
